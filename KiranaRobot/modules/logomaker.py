...
from KiranaRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./KiranaRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Kirana Robot](https://t.me/EmiexRobot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./KiranaRobot/resources/fonts"):
            os.remove(font_)
